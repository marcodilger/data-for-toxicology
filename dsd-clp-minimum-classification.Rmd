...
plot_substances_of_concern <- data_oral_extended %>% 
  ...
  ggplot(aes(x = fct_reorder(substance, single_category, min), y = category_from_exp)) + 
 # geom_count() +
  geom_count(aes(size = ..n..,
                 colour = as.factor(numeric_clp_class))) + # fct_reorder to order factor from lowest single_category to highest
  ...
  labs(color = "original\nclassification",
       size = "countsize" # <- this does not work. "size" probably not correct for adressing the legend TODO
       )
