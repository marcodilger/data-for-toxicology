...
A possibility is to use the EC inventory and do a lookup. However the EC invnetory export is limited to 20000 entires.
...
data_oral_extended %>% 
  ...
  head
```
Suspicion: there are some entries where the unit is not the same, thus severe differences in effect levels
```{r}
data_oral_extended %>% 
  pull(unit) %>% 
  table
...
### peracetic acid  
High number of studies, several RL1. UUIDs also don't match for this substance. I didn't check all study records in the dossier, but there is one very obvious problem here: in addition to the LD50 of the pure substance, also LD50 of formulations are reported (with consequently considerably higher LD50 values). Somehow these values don't show up in eChemPortal. Actually, it seems the data from eChemPortal contains only one LD50 value from each of the study records... :/
...
### why do I loose data from eCP?

```{r}
studies_oral %>% 
  filter(str_detect(`Substance Name`, "cyclohexanamine")) %>% 
  pull(Values)
```
