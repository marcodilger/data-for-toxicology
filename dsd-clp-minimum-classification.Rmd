...
4 experimental values from 2 studies. All values quite clearly indicating cat. 1 instead of 2, the current CLH category.
ToDo: comparison with dossier
...
data_oral_extended %>% 
  ...
  arrange(single_category, substance, reliability)
```
3 experimental values exported from eChemportal, from 2 studies. Same reliability, first study -> cat. 3, second study -> cat. 2. 
ToDo: check dossier
...
data_oral_extended %>% 
  ...
  arrange(single_category, substance, reliability)
```
1 RL 1 study indicating cat 3. 3 RL2 studies with values indicating cat 2/3. Current CLH: cat. 3, seems justified.
