...
data_oral_extended %>% 
  ...
  arrange(single_category, substance, reliability)
```
CLH cat 4 seems justified. ToDo: Dossier
...
data_oral_extended %>% 
  ...
  arrange(single_category, substance, reliability)
```
2 RL1 studies with diverging classification results, difficult case that probably can't be judged correctly by just looking at the pure values. ToDo: check dossier for metal content.
...
data_oral_extended %>% 
  ...
  arrange(single_category, substance, reliability)
```
All available results from eChemportal are RL2, several indicating cat 3 instead of cat. 4 (current CLH), shoudl probbaly be classified as 3. ToDo: Dossier
