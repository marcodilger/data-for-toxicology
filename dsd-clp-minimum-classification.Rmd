...
# todo: find way to identify entries that can't be identified by cas (because of NA e.g.) by name instead.
...
There shouldn't be any entries witout EC number actually because we're using ECHA dissemiated data here. All entires should have at least an EC number.

ToDo: eChemPortal epxort appears to have a lot of missing names.
...
No, those are removed.

```{r}
studies_oral_name <- studies_oral_name %>% 
  filter(!(is.na(cas_echa) & is.na(ec_echa)))
```
# Todo: combine the cleaned lists
